...
public class SpawnHyperlinks : MonoBehaviour
{
    ...
    private GithubHyperlinkDataFetcher githubHyperlinkDataFetcher = new GithubHyperlinkDataFetcher();

    private ARTrackedImage currentlyTrackedARImage;

    // A List to store the hyperlink url and spatial coordinates of each AR hyperlink overlay
    ...
    // This application technically supports pulling hyperlink information
    // from either a preset Github repo, or from a preset JsonBin.io account.
    // For now, we are primarily using JsonBin, but the support is here
    // for a switch to Github to be made relatively easily
    ...
    private void FetchHyperlinkData(string id, int pageNum) {
        if (hyperlinkDataSource == DataSource.GitHub) {
            StartCoroutine(githubHyperlinkDataFetcher.FetchJSONFromId(id, pageNum, ProcessHyperlinkData));
        } else {
        ...
        for (int i = 0; i < arDocumentData.pages.Count; i++)
        {
            ...
            if (i == pageNum)
            {
                ...
                foreach (Hyperlink hyperlink in page.hyperlinks)
                {
                    ...
                    OverlayData newOverlayData = new OverlayData(arDocumentData.URL, hyperlink.uri, scaleVector3, offsetVector3);
    ...
    private void SpawnOverlays() {
        ...
        {
#nullable enable
            string? originalWebsiteUrl = overlayData.originalWebsiteUrl;
#nullable disable
            string hyperlinkUrl = overlayData.hyperlinkUrl;
            Vector3 offset = overlayData.offset;
            Vector3 scale = overlayData.scale;

            GameObject hyperlinkOverlay;
            if (URLAnalyzer.HasImageExtension(hyperlinkUrl)) {
                hyperlinkOverlay = Instantiate(imageHyperlinkOverlayPrefab, currentlyTrackedARImage.transform);
            } else if (URLAnalyzer.HaveSameRootDomain(hyperlinkUrl, originalWebsiteUrl)) {
